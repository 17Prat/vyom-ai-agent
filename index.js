...
const DECISION_SYSTEM_PROMPT = `Tu Vyom Automations hai — Vyom Automations ka official Content Intelligence, Personalized Recommendation Agent aur Reel Creation Assistant.
...
- Content Type, Emotional Tone, Knowledge Level, Related Topics, Related Vyom Automations Features.
...
## 5. CONTEXTUAL VYOM AUTOMATIONS FEATURE MATCHING
...
**Agar aapko chanting mein interest hai toh Vyom Automations ke yeh features useful ho sakte hain:**
...
**Agar aapko temples mein interest hai toh Vyom Automations ke yeh features useful ho sakte hain:**
...
**Vyom Automations Install Kaise Karein:**

**For Android (Google Play Store):**
1. Play Store kholiye aur search karein: **Vyom Automations**
2. **Install** button tap karein.
3. Link: https://play.google.com/store/apps/details?id=com.brahmand.app

**For iOS (Apple App Store):**
1. App Store kholiye aur search karein: **Vyom Automations**
      ...
      case 'check_and_reply_instagram_messages': {
        ...
        if (checkResult.lastMessage && checkResult.isIncoming) {
          ...
          const replyPrompt = [
            {
              role: 'system',
              content: `Tu Vyom Automations hai — Vyom Automations ka official ultra-intelligent, friendly, aur highly empathetic assistant.
...
async function planInstagramReel(topic) {
  ...
  const llmPrompt = `You are Vyom Automations — India's best Instagram Reel script writer.
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const isPosterRequest = cleanMsg.startsWith('/meta') ||
      ...
      cleanMsg.includes('nakshatra poster') || cleanMsg.includes('vyom poster') || cleanMsg.includes('vyom automations poster') ||
    ...
    if (isPosterRequest) {
      ...
      if (uploadedImage) {
        try {
          ...
          masterPosterUrl = `/posters/${filename}`;
          reply = "Here is your custom **Vyom Automations Master Promotional Poster** generated from your uploaded photo! 🪷✨\n\nShowcasing your uploaded screenshot in ultra-high quality, ready for Meta Ads & Instagram marketing!";
        } catch (e) {
    ...
    if (cleanMsg === 'confirm and post reel to instagram' || cleanMsg === 'confirm & post reel to instagram') {
      ...
      const lastCaption = getSessionData(sessionId, 'last_video_caption') || getSessionData(sessionId, 'last_image_caption') || 'Vyom Automations Reel #VyomAutomations';
...
Introduce the premium Sanatan/heritage app "Vyom Automations". Keep it under 2 sentences.
