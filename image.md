...
Launch one low-reasoning explorer sub-agent with the aesthetic-expression-for-concepts prompt from subagent-prompts.md.
...
Launch one low-reasoning worker or explorer sub-agent with the danbooru-download-fit-review prompt from subagent-prompts.md.
...
Launch one low-reasoning worker or explorer sub-agent with the gelbooru-download-fit-review prompt from subagent-prompts.md.
...
Use the matching prompt template from subagent-prompts.md:
...
Critical Rules
Main creates the rough three concepts before research sub-agents run.
Do not use a prompt-draft sub-agent; Main writes final prompts.
Danbooru and Gelbooru must be separate sub-agents.
Danbooru/Gelbooru sub-agents review each image for fit against concept_01 to concept_03.
Danbooru/Gelbooru download queries must exclude explicit and questionable ratings before download.
Reference images are optional and selective; use only images that fit the user request and the assigned prompt.
If reference images are selected, generation sub-agents must attach them during image generation.
If no reference image fits, generate with text only and record that no references were used.
Generate images with the imagegen skill after final prompts are written.
Run image generation in parallel, one sub-agent per prompt.
Run image review in parallel, one sub-agent per prompt result.
Save every generated image locally under reference/generated/<prompt_id>/, accepted or rejected.
No proper nouns in final prompt output. See style-replacements.md.
Quality Checklist
